...
void next()   // This function reads the next token from the source code and handles different types of input.
{
  ...
  while (tk = *p) { // tk is the current token, and *p is a pointer to the current position. So this loop reads the next character from the source code.
    ...
    // Handle increment and addition operators
    else if (tk == '+') { if (*p == '+') { ++p; tk = Inc; } else tk = Add; return; }
    // Handle decrement and subtraction operators
    ...
    // Handle bitwise OR and AND operators
    else if (tk == '|') { if (*p == '|') { ++p; tk = Lor; } else tk = Or; return; }
    else if (tk == '&') { if (*p == '&') { ++p; tk = Lan; } else tk = And; return; }
    else if (tk == '^') { tk = Xor; return; } // Set token to bitwise XOR operator
    else if (tk == '%') { tk = Mod; return; } // Set token to modulo operator
    else if (tk == '*') { tk = Mul; return; } // Set token to multiplication operator
    else if (tk == '[') { tk = Brak; return; } // Set token to array bracket operator
    else if (tk == '?') { tk = Cond; return; }  // Set token to conditional operator
    
    // Ignore other characters like semicolon, braces, parentheses.....
...
void expr(int lev)
{
  ...
  while (tk >= lev) { // "precedence climbing" or "Top Down Operator Precedence" method (Parsing expressions of operators with different precedence levels)
    ...
    // Logical OR (||): If the first operand is true (nonzero), short-circuit and skip the second operand
    else if (tk == Lor) { next(); *++e = BNZ; d = ++e; expr(Lan); *d = (int)(e + 1); ty = INT; } 
    // Logical AND (&&): If the first operand is false (zero), short-circuit and skip the second operand
    else if (tk == Lan) { next(); *++e = BZ;  d = ++e; expr(Or);  *d = (int)(e + 1); ty = INT; } 
     // Bitwise OR (|): Push left operand, evaluate right operand, apply OR operation
    else if (tk == Or)  { next(); *++e = PSH; expr(Xor); *++e = OR;  ty = INT; }
    // Bitwise XOR (^): Push left operand, evaluate right operand, apply XOR operation
    else if (tk == Xor) { next(); *++e = PSH; expr(And); *++e = XOR; ty = INT; } 
    // Bitwise AND (&): Push left operand, evaluate right operand, apply AND operation
    else if (tk == And) { next(); *++e = PSH; expr(Eq);  *++e = AND; ty = INT; } 
     // Equality comparison (==): Push left operand, evaluate right operand, check if they are equal
    else if (tk == Eq)  { next(); *++e = PSH; expr(Lt);  *++e = EQ;  ty = INT; }
     // Inequality comparison (!=): Push left operand, evaluate right operand, check if they are not equal
    else if (tk == Ne)  { next(); *++e = PSH; expr(Lt);  *++e = NE;  ty = INT; }
     // Less than (<): Push left operand, evaluate right operand, check if left < right
    else if (tk == Lt)  { next(); *++e = PSH; expr(Shl); *++e = LT;  ty = INT; }
    // Greater than (>): Push left operand, evaluate right operand, check if left > right
    else if (tk == Gt)  { next(); *++e = PSH; expr(Shl); *++e = GT;  ty = INT; } 
    // Less than or equal (<=): Push left operand, evaluate right operand, check if left <= right
    else if (tk == Le)  { next(); *++e = PSH; expr(Shl); *++e = LE;  ty = INT; }
     // Greater than or equal (>=): Push left operand, evaluate right operand, check if left >= right 
    else if (tk == Ge)  { next(); *++e = PSH; expr(Shl); *++e = GE;  ty = INT; }
     // Bitwise left shift (<<): Push left operand, evaluate right operand, apply left shift
    else if (tk == Shl) { next(); *++e = PSH; expr(Add); *++e = SHL; ty = INT; }
    // Bitwise right shift (>>): Push left operand, evaluate right operand, apply right shift
    ...
    else if (tk == Mul) { next(); *++e = PSH; expr(Inc); *++e = MUL; ty = INT; } // Move to the next token, push the left operand and perform multiplication
    else if (tk == Div) { next(); *++e = PSH; expr(Inc); *++e = DIV; ty = INT; } // Move to the next token, push the left operand and perform division
    else if (tk == Mod) { next(); *++e = PSH; expr(Inc); *++e = MOD; ty = INT; } // Move to the next token, push the left operand and perform modulo operation
    else if (tk == Inc || tk == Dec) {  // Handle increment and decrement operators (POST)
      if (*e == LC) { *e = PSH; *++e = LC; } // If current value is a character variable, push it
      else if (*e == LI) { *e = PSH; *++e = LI; }  // If current value is an integer variable, push it
      else { printf("%d: bad lvalue in post-increment\n", line); exit(-1); } // Ensure it's a valid lvalue for increment/decrement
      *++e = PSH; *++e = IMM; *++e = (ty > PTR) ? sizeof(int) : sizeof(char); // Push the size based on type (int or char)
      *++e = (tk == Inc) ? ADD : SUB; // Perform increment or decrement based on the operation
      *++e = (ty == CHAR) ? SC : SI;  // Store the incremented or decremented value back in memory
      *++e = PSH; *++e = IMM; *++e = (ty > PTR) ? sizeof(int) : sizeof(char);  // Push the size for reverse operation
      *++e = (tk == Inc) ? SUB : ADD;  // Reverse the increment/decrement to maintain original value
